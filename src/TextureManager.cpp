...
void TextureManager::loadAlpha(std::string fileName, std::string id, SDL_Renderer* pRenderer)
{
	SDL_Surface* pTempSurface = IMG_Load(fileName.c_str());
	SDL_SetColorKey(pTempSurface, 1, SDL_MapRGB(pTempSurface->format, 0, 255, 0));
	SDL_Texture* pTexture = SDL_CreateTextureFromSurface(pRenderer, pTempSurface);
